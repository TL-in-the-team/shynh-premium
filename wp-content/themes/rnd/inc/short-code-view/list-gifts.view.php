...
<div class="news-news__list">
  <?php 
    foreach($list_gifts as $post): setup_postdata($post); 
      $gift_id = get_the_id();
      $post_thumbnail_url = get_the_post_thumbnail_url($post_id,'news-thumb');
  ?>
    <div class="news-news__item gift">
        <a href="<?php the_permalink();?>" class="news-news__item__img-link">
        <img src="<?php echo imageEncode('/images/no-image-gift.webp'); ?>" data-src="<?php echo imageEncodeURL($post_thumbnail_url) ?>" alt="ALT" title="title" class="news-news__item__img lazy"></a>
